    ...
    *******
    ...
    with PdfPages( file_output ) as pdf:
        ...
        for i in range( T ):
            ...
            op1 = open( file_root + str( i ) + '.dat', 'r' )
            lin1 = op1.readlines()
            op1.close()
            ...
            if i > 0:
                w1 = numpy.loadtxt( file_root + str( i ) + 'weights.dat' )
            else:
            ...
            ax1.set_xlim( Parameters['param_lim']-0.1, Parameters['param_lim'][0][1]+0.1)
        ...
        convergence = numpy.array( [ float(M)/item  for item in ndraws_ev ] )
    
        #Plot epsilon evolution
        plt.figure()
        plt.scatter( range( n_tries ), numpy.array(epsilon_ev), color='red', marker='*', label='distance threshold' )
        plt.legend()
        ...
        plt.figure()
        plt.scatter( range( n_tries ), numpy.array(time_ev), color='blue', marker='o', label='time' )
        ...
        plt.figure()
        plt.scatter( range( n_tries ), convergence/max( convergence ), color='green', marker='x', label='convergence' )
        plt.legend()
        plt.xlabel( 'Particle System' )
