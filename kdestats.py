...
def confmap(map, frac):
    ...
    def diffsum(level, map, ndesired):
        return ((map[map >= level].sum()/map.sum() - ndesired))

    if hasattr(frac,'__iter__'):
        return [confmap(map,thisfrac) for thisfrac in frac]
    ...
    return bisect(diffsum, map.min(), map.max(), args=(map, frac))
