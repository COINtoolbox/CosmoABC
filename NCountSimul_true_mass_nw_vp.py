...
def choose_surv_par( summary_fid, mass_bin, quantile_list, tolerance, n_tries, ncpu, CP, zmin, zmax, area,  seed ):
    """
    ...
    ncount=NCountSimul (zmin, zmax, numpy.log ( mass_bin[0] ), numpy.log ( 10**16 ), area )
   ...
   # 2*numpy.dot( numpy.transpose( par_surv[:,:Nparams] ),numpy.dot( numpy.diag( weights ) , par_surv[:,:Nparams] ) )
    ...
    return   [numpy.array( par_list )  ,  numpy.array( indx_list ,dtype=int)   , numpy.array( par_cov )]
